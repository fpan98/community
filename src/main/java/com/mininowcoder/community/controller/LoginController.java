...
public class LoginController implements CommunityConstant {
    ...
    @Autowired
    private Producer kaptchaProduder;
    ...
    public String activation(Model model,
        ...
        if(result==ACTIVATION_SUCCESS){
            ...
            model.addAttribute("target", "/login");//跳转至登录页面
    ...
    public void getKaptcha(HttpServletResponse response/*, HttpSession session*/){
        // 生成验证码
        String text = kaptchaProduder.createText();
        BufferedImage image = kaptchaProduder.createImage(text);
        // 将验证码存入session
        // session.setAttribute("kaptcha", text); // 重构用redis存储
    ...
    public String login(String username, String password, String code, boolean rememberme,
        ...
        // 重构后从redis中取
        String kaptcha = null;
        if(StringUtils.isNoneBlank(kaptchaOwner)){ // 判断cookie中的kaptchaOwner是否失效
            String redisKey = RedisKeyUtil.getKaptchaKey(kaptchaOwner);
