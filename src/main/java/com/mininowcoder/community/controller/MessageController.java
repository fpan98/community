    ...
    public String getLetterList(Model model, Page page) {
        ...
        if (conversationList != null) {
            for (Message message : conversationList) {
                ...
                map.put("unreadCount", messageService.findLetterUnreadCount(user.getId(), message.getConversationId()));// 每个会话未读私信数量
                int targetId = user.getId() == message.getFromId() ? message.getToId() : message.getFromId();
                map.put("target", userService.findUserById(targetId)); // target用户发送的私信
    ...
    public String getNoticeDetail(@PathVariable("topic") String topic, Page page, Model model){
        ...
        if(noticeList!=null){
            for (Message notice : noticeList) {
                ...
                map.put("postId", data.get("postId"));// 评论和回复是对应帖子的id。关注postId=null
                // 通知的作者-都是系统通知
